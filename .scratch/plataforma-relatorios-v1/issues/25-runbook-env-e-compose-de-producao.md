...
- [ ] Runbook documenta que Jaeger, Grafana e Prometheus também não são expostos, e como a Operação alcança os três pela rede interna (ADR-0017)
...
- [ ] Runbook documenta os volumes do Jaeger (Badger) e do Loki (filesystem) — backup, restauração e o fato de que perdê-los perde o histórico de traces e logs, sem HA (ADR-0017)
- [ ] Runbook documenta o caminho de investigação: erro relatado pelo usuário → log no Grafana/Loki → trace no Jaeger pelo `traceId` (ADR-0017)
