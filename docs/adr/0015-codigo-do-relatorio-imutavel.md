...
O Código do Relatório valida contra `^[A-Z]{2,20}-\d{4}$`; a sigla precisa corresponder a um Produto já cadastrado (chave estrangeira, não texto livre); o número vai de 0001 a 9999, sendo 0000 inválido. O Código é imutável após a criação e **nunca é reaproveitado**, mesmo depois da exclusão do Relatório.
...
- A sigla não pode conter hífen, porque o hífen é o separador (daí `CONTACORRENTE`, não `CONTA-CORRENTE`).
