...
- Obrigatório em contrapartida: verificação de e-mail antes de a conta ser utilizável, proteção contra bots no formulário de cadastro (reCAPTCHA do Keycloak), rate limiting, política de senha, TLS + WAF e uma rotina de limpeza das contas que nunca recebem Role de Relatório. **A verificação de e-mail está ligada no realm mas não é entregue**: o ADR-0020 escolheu Mailpit como destino em todos os ambientes, de modo que produção não envia e-mail e o autocadastro não se completa. A limpeza de contas e o rate limiting ganham peso enquanto isso durar.
